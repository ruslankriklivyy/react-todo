...
import { TasksItems, TasksItemsRow, TasksCheckbox, TaskAction } from './styled';

const Task = ({ list, onRemove, onCompleted, withoutEmpty, onEdit }) => {
  const onCompletedChange = (listId, taskId, completed) => {
    onCompleted(listId, taskId, completed);
  };

  return (
    <TasksItems>
      {!withoutEmpty && !list && !list.tasks && !list.tasks.length && <h2>Задачи отсутствуют</h2>}
      {list.tasks &&
        list.tasks.map((item) => {
          return (
            <TasksItemsRow key={item.id}>
              <TasksCheckbox>
                <input
                  onChange={(e) => onCompletedChange(list.id, item.id, e.target.checked)}
                  id={`task-${item.id}`}
                  type="checkbox"
